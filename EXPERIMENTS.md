...
## 9. Валідація моделі прогнозування продуктивності

**Файл:** `notebooks/02_model_training_validation.ipynb`

Після навчання моделей прогнозування продуктивності у notebook виконано додаткову валідацію якості на тестовій вибірці. Мета цього етапу - не лише зафіксувати числові метрики, а й перевірити, наскільки стабільними є прогнози моделі та де виникають найбільші помилки.

У notebook виконано:

- порівняння моделей `Linear Regression`, `Gradient Boosting` і `MLP`;
- перевірку, що `Linear Regression` має значно вищий MAE;
- аналіз того, що результати для `Gradient Boosting` і `MLP` дуже близькі;
- розрахунок додаткових метрик валідації;
- аналіз розподілу абсолютної помилки;
- побудову графіка відповідності між `actual_latency` і `predicted_latency`;
- формування коротких висновків щодо якості моделі.
...
**Висновок:** модель придатна для попереднього ranking конфігурацій, але окремі прогнози можуть мати помітну похибку. Найбільші помилки спостерігаються для `raspi4` та `pixel3`, тоді як для `eyeriss`, `fpga` та `edgetpu` прогнозування стабільніше. Це означає, що модель краще використовувати як інструмент попереднього відбору hardware-конфігурацій, а не як заміну фінальному benchmark-вимірюванню на реальному пристрої.
